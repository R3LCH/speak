...
X11 requires `xclip` and `xdotool` (`sudo apt install xclip xdotool`). Wayland requires `wl-clipboard`, `ydotool`, the GlobalShortcuts portal where supported, and permission to read `/dev/input/event*` and write `/dev/uinput` (usually via udev rules or group membership). Run `speak doctor` to inspect the session and dependencies.
